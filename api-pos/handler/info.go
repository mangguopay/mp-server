...
func (s *AuthHandler) ServicerBills() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		}, "page", "page_size", "start_time", "money_type", "bill_type", "op_no", "order_status")
	}
}
...
func (*AuthHandler) AddCashier() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoUpdate(c, func(params interface{}) (string, interface{}, error) {
			accountType := inner_util.GetJwtDataString(c, "account_type")
			if accountType != constants.AccountType_SERVICER {
				ss_log.Error("登陆的账号角色为[%v],无权限调用此api", accountType)
				return ss_err.ERR_SYS_NO_API_AUTH, nil, nil
			}

			req := &go_micro_srv_auth.AddCashierRequest{
				...
				ServicerNo:  inner_util.GetJwtDataString(c, "iden_no"),
			}

			if errStr := verify.AddCashierVerify(req); errStr != "" {
				return errStr, nil, nil
			}
...
func (s *AuthHandler) GetCashiers() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGetList(c, func(params []string) (string, interface{}, int32, error) {
			accountType := inner_util.GetJwtDataString(c, "account_type")
			if accountType != constants.AccountType_SERVICER {
			...
			reply, err := CustHandlerInst.Client.GetCashiers(context.TODO(), &go_micro_srv_cust.GetCashiersRequest{
				Page:       strext.ToInt32(params[0]),
				PageSize:   strext.ToInt32(params[1]),
				ServicerNo: inner_util.GetJwtDataString(c, "iden_no"),
			})
...
func (s *AuthHandler) GetCashierDetail() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGetList(c, func(params []string) (string, interface{}, int32, error) {
			reply, err := CustHandlerInst.Client.GetCashierDetail(context.TODO(), &go_micro_srv_cust.GetCashierDetailRequest{
				CashierNo: strext.ToStringNoPoint(params[0]),
			})
			return reply.ResultCode, reply.Datas, 0, err
		}, "cashier_no")
	}
...
func (s *AuthHandler) DeleteCashier() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoUpdate(c, func(params interface{}) (string, interface{}, error) {
			accountType := inner_util.GetJwtDataString(c, "account_type")
			if accountType != constants.AccountType_SERVICER {
			...
			req := &go_micro_srv_cust.DeleteCashierRequest{
				CashierNo:     container.GetValFromMapMaybe(params, "cashier_no").ToStringNoPoint(),
				ServicerAccNo: inner_util.GetJwtDataString(c, "account_uid"),
			}
...
func (s *AuthHandler) ModifyCashier() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoUpdate(c, func(params interface{}) (string, interface{}, error) {
			//此接口已废除，后台只允许帮服务商创建和删除店员，不允许更改店员的手机号
			ss_log.Error("接口已废除，后台服务商创建、删除店员，不允许更改店员的手机号")
			...
			//}
			//
			//req := &go_micro_srv_cust.ModifyCashierRequest{
			//	CashierNo: container.GetValFromMapMaybe(params, "cashier_no").ToStringNoPoint(),
			//	Phone:     container.GetValFromMapMaybe(params, "phone").ToStringNoPoint(),
			//}
...
// 存款后的订单详情
func (s *AuthHandler) ChangeBalanceDetail() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGet(c, func(params interface{}) (string, gin.H, error) {
			req := &go_micro_srv_cust.ChangeBalanceDetailRequest{
				OrderNo: strext.ToString(params),
			}

			if errStr := verify.ChangeBalanceDetailVerify(req); errStr != "" {
				return errStr, nil, nil
			}

			reply, err := CustHandlerInst.Client.ChangeBalanceDetail(context.TODO(), req)
			return reply.ResultCode, gin.H{
				"data": reply.Data,
			}, err
		}, "order_no")
