...
func (s *AuthHandler) MobileLogin() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGet(c, func(params interface{}) (string, gin.H, error) {
			...
			//账号就是手机号，但又可能柬埔寨用户输多一个0再前面，所以要去掉
			...
			if reply.AccountUid != "" {
				...
				errCode := dao.AccDaoInstance.InsertPosLoginToken(reply.AccountUid, "", loginToken, 1, c.ClientIP())
...
func (s *AuthHandler) RegSms() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoUpdate(c, func(params interface{}) (string, interface{}, error) {
			...
			//功能,注册-reg;找回密码-backpwd;修改支付密码-paypwd;验证手机号-checkphone;更改手机号-modifyphone;手机号取款-withdraw
			if !util.InSlice(function, []string{common.SmsTempKeyReg, common.SmsTempKeyBackPwd, common.SmsTempKeyPayPwd,
				common.SmsTempKeyCheckPhone, common.SmsTempKeyModifyPhone, common.SmsTempKeyWithDraw}) {
				ss_log.Error("不支持此类短信模板，function=%v ", function)
			...
			req := &go_micro_srv_cust.RegSmsRequest{
				...
				Function: function,
				Lang:     ss_net.GetCommonData(c).Lang,
				//功能,注册-reg;找回密码-backpwd;修改支付密码-paypwd;验证手机号-checkphone;更改手机号-modifyphone;
				PubKey:      container.GetValFromMapMaybe(params, "pub_key").ToStringNoPoint(),
				CountryCode: countryCode,
			}
...
func (*AuthHandler) GetAgreementAppDetail() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGetList(c, func(params []string) (string, interface{}, int32, error) {
			reply, err := CustHandlerInst.Client.GetAgreementAppDetail(context.TODO(), &go_micro_srv_cust.GetAgreementAppDetailRequest{
				Lang: strext.ToStringNoPoint(params[0]),
...
func (*AuthHandler) GetAppConsultationConfigDetail() gin.HandlerFunc {
	return func(c *gin.Context) {
		ss_net.NetU.DoGetList(c, func(params []string) (string, interface{}, int32, error) {
			reply, err := CustHandlerInst.Client.GetAppConsultationConfigDetail(context.TODO(), &go_micro_srv_cust.GetAppConsultationConfigDetailRequest{
				Lang: ss_net.GetCommonData(c).Lang,
			})
