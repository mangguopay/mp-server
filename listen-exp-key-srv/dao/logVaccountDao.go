...
func (LogVaccountDao) InsertLogTx(tx *sql.Tx, vaccountNo, opType, amount, bizLogNo string) string {
	...
	err := ss_sql.ExecTx(tx, `insert into log_vaccount(log_no,create_time,vaccount_no,amount,op_type,frozen_balance,balance,reason,biz_log_no) values ($1,current_timestamp,$2,$3,$4,$5,$6,$7,$8)`,
		strext.GetDailyId(), vaccountNo, amount, opType, fbalance, balance, constants.VaReason_Exchange, bizLogNo)
	if nil != err {
