...
type StatisticUserTransferDao struct{}

var StatisticUserTransferDaoInst StatisticUserTransferDao

type StatisticUserTransferData struct {
	Day          string
	CurrencyType string
	TotalNum     int64
	TotalAmount  int64
...
// 获取转账的统计数据
func (s *StatisticUserTransferDao) GetStatisticData(startDate string, endDate string, currencyType string) ([]StatisticUserTransferData, error) {
	...
	sqlStr := "SELECT day, SUM(num) AS total_num, SUM(amount) AS total_amount, SUM(fee) AS total_fee FROM statistic_user_transfer "
	...
	list := []StatisticUserTransferData{}
	for rows.Next() {
		...
		list = append(list, StatisticUserTransferData{
...
// 获取转账的统计数据-列表
func (s StatisticUserTransferDao) GetStatisticDataList(req *go_micro_srv_cust.GetStatisticUserTransferListRequest) ([]*go_micro_srv_cust.StatisticUserTransferData, int32, error) {
	dbHandler := db.GetDB(constants.DB_CRM)
	defer db.PutDB(constants.DB_CRM, dbHandler)
	...
	sqlCnt := "SELECT count(1) FROM statistic_user_transfer " + whereModel.WhereStr
	...
	sqlStr := "SELECT id, day, ctype, num, amount, fee, create_time, update_time FROM statistic_user_transfer " + whereModel.WhereStr
	...
	var dataList []*go_micro_srv_cust.StatisticUserTransferData

	for rows.Next() {
		var data go_micro_srv_cust.StatisticUserTransferData
		err = rows.Scan(
			&data.Id,
			&data.Date,
			&data.CurrencyType,
...
func (r *StatisticUserTransferDao) Insert(wc *DataCount) error {
	dbHandler := db.GetDB(constants.DB_CRM)
	defer db.PutDB(constants.DB_CRM, dbHandler)

	id := strext.GetDailyId()
	return ss_sql.Exec(dbHandler, `INSERT INTO statistic_user_transfer(id,day,ctype,num,amount,fee,create_time,update_time)
			VALUES($1,$2,$3,$4,$5,$6,current_timestamp,current_timestamp)
			ON CONFLICT ON CONSTRAINT unq_transfer_row
			DO UPDATE SET ctype = EXCLUDED.ctype,num = EXCLUDED.num,
			amount = EXCLUDED.amount,fee = EXCLUDED.fee,update_time = current_timestamp`,
		id, wc.Day, wc.CType, wc.Num, wc.Amount, wc.Fee)
}
