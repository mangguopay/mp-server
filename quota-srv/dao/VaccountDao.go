...
func (VaccountDao) GetVaccountNoTxV2(tmProxy *ss_struct.TmServerProxy, accountNo string, vaType int32) (string, error) {

	sql := "select vaccount_no from vaccount where account_no=$1 and va_type=$2 and is_delete='0' limit 1"
	args := []string{accountNo, strext.ToStringNoPoint(vaType)}
	...
	return rsp.Datas["vaccount_no"], nil
...
func (VaccountDao) InitVaccountNoTxV2(tmProxy *ss_struct.TmServerProxy, accountNo string, vaType int32) (string, error) {
	vaccountNo := strext.NewUUID()
	var balanceType string
	switch vaType {
	case constants.VaType_USD_DEBIT:
	...
	sql := `insert into vaccount(vaccount_no,account_no,va_type,balance,create_time,is_delete,use_status,frozen_balance,balance_type) ` +
		` values ($1,$2,$3,$4,current_timestamp,$5,$6,$7,$8)`
	args := []string{vaccountNo, accountNo, strext.ToStringNoPoint(vaType), "0", "0", "1", "0", balanceType}
...
// 修改虚拟账户余额，余额必须正
func (VaccountDao) ModifyVaccRemainUpperZero(tx *sql.Tx, vaccountNo, amount, op, logNo, reason string) (errCode string) {
	switch op {
	case "+":
	...
	errCode = LogVaccountDaoInst.InsertLogTx(tx, vaccountNo, constants.VaOpType_Add, amount, logNo, reason)
	if errCode != ss_err.ERR_SUCCESS {
...
func (VaccountDao) ModifyVaccRemainAndFrozenUpperZero(tx *sql.Tx, vaccountNoFrom, amount, logNo, reason string) (errCode string) {
...
// 修改虚拟账户余额，余额不必须为0
func (VaccountDao) ModifyVacc(tx *sql.Tx, vaccountNo, amount, op, logNo, reason string) (errCode string) {
	switch op {
	...
	case constants.VaOpType_Balance_Frozen_Add:
		err := ss_sql.ExecTx(tx, `update vaccount set balance=balance+$1,frozen_balance=frozen_balance+$1, modify_time = current_timestamp where vaccount_no=$2 and is_delete='0'`, amount, vaccountNo)
		if nil != err {
			ss_log.Error("err=%v", err)
			return ss_err.ERR_PAY_AMT_NOT_ENOUGH
		}
	case constants.VaOpType_Balance_Defreeze_Add:
		err := ss_sql.ExecTx(tx, `update vaccount set balance=balance-$1,frozen_balance=frozen_balance-$1, modify_time = current_timestamp where vaccount_no=$2 and is_delete='0'`, amount, vaccountNo)
		if nil != err {
			ss_log.Error("err=%v", err)
			return ss_err.ERR_PAY_AMT_NOT_ENOUGH
	...
	errCode = LogVaccountDaoInst.InsertLogTx(tx, vaccountNo, op, amount, logNo, reason)
	if errCode != ss_err.ERR_SUCCESS {
		ss_log.Error("err=%v", errCode)
		return errCode
	}
...
func (VaccountDao) ModifyVaccV2(tmProxy *ss_struct.TmServerProxy, vaccountNo, amount, op, logNo, reason string) error {
	sql := ""
	args := []string{}
	switch op {
	case constants.VaOpType_Add:

		sql = `update vaccount set balance=balance+$1, modify_time = current_timestamp where vaccount_no=$2 and is_delete='0'`
	...
	case constants.VaOpType_Freeze:
		...
		sql = `update vaccount set balance=balance-$1,frozen_balance=frozen_balance+$1, modify_time = current_timestamp where vaccount_no=$2 and is_delete='0'`
		args = []string{amount, vaccountNo}
	case constants.VaOpType_Defreeze_But_Minus:

		sql = `update vaccount set frozen_balance=frozen_balance+$1, modify_time = current_timestamp where vaccount_no=$2 and is_delete='0'`
		args = []string{amount, vaccountNo}
	case constants.VaOpType_Defreeze:
	...
	default:
		return errors.New(fmt.Sprintf("操作类型错误,op: %v", op))
	}
	...
	err2 := LogVaccountDaoInst.InsertLogTxV2(tmProxy, vaccountNo, op, amount, logNo, reason)
	if err2 != nil {
		return err2
	}
