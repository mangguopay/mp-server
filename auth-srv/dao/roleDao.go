 ...
 * 获取角色列表
 */
func (RoleDao) GetRoleList(roleName, accountType, masterAcc string, pageSize, page int32) (errCode string, datas []*auth.RoleData) {
	dbHandler := db.GetDB(constants.DB_CRM)
	defer db.PutDB(constants.DB_CRM, dbHandler)
	...
	for rows.Next() {
		...
		err := rows.Scan(&data.RoleNo, &data.RoleName, &data.CreateTime, &data.ModifyTime, &data.AccType, &data.DefType,
			&masterAcc, &masterAccount)
		if err != nil {
			ss_log.Error("err=[%v]", err)
			continue
		}
