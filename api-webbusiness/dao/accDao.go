...
func (*AccDao) DeleteLoginToken(accNo string) error {
	dbHandler := db.GetDB(constants.DB_CRM)
	defer db.PutDB(constants.DB_CRM, dbHandler)
...
func (r *AccDao) GetLoginToken(accNo, loginToken, urlPath, xRealIp string) (isTimeout bool, isReset bool) {
	...
	if ip.String != xRealIp && ip.String != "" {
		ss_log.Error("登录ip改变, ip[%v], xRealIp[%v]", ip.String, xRealIp)
		//return false, false
	}
	...
	if lastOpTime.String != "" && loginTime.String != "" {
		...
		if curTimeT.After(loginTimeT.Add(55*time.Minute)) && curTimeT.Before(lastOpTimeT.Add(5*time.Minute)) {
			ss_log.Error("login-reset,accNo:%v, loginToken:%v, curTimeT:%v, loginTimeT:%v, loginTime:%v, lastOpTimeT:%v, lastOpTime:%v",
				accNo, loginToken, curTimeT, loginTimeT, loginTime, lastOpTimeT, lastOpTime)
			isReset = true
		}
	}
