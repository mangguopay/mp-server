...
func (*JwtVerifyMw) VerifyToken() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if strings.HasPrefix(token, "Bearer ") {
			...
			if isOk == false {
				ss_log.Info("%v|jwt验证失败,xRealIp:%v, value:%v", traceNo, xRealIp, value)
			}
			...
			if isOk {
				...
				if c.Request.RequestURI != "/auth/logout" {
					...
					//isReset = true
					if isReset {
						ss_log.Info("%v|jwt-isReset,accNo:%v,xLoginToken:%v,xPath:%v,xRealIp:%v", traceNo,
							accNo, strext.ToStringNoPoint(xLoginToken), xPath, xRealIp)
